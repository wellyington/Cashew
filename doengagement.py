...
def doengagement(username, password, hashtag, limit):
	...
	while engagements != engLimit:
		try:
			...
			sqlCheck = "Select username, url From engagement where username = %s and url = %s"
			...
			if row_count == 0:
				try:
					...
					sql = "INSERT INTO `engagement`(`ID`,`hashtag`,`username`,`profile`,`url`,`date`,`time`) VALUES(%s,%s,%s,%s,%s,%s,%s)"
